...
def summarize_data():
    ...
    for x in range(0, len(df_agg_views), 7):
        MD_SUMMARY.write(
            textwrap.dedent(
                f"""
        | {df_agg_views["time"].iloc[x].split(" ")[0]} | {round(df_agg_clones["clones_total"].iloc[x:x+7].mean(), 2)}|{round(df_agg_clones["clones_unique"].iloc[x:x+7].mean(), 2)}|{round(df_agg_views["views_total"].iloc[x:x+7].mean(), 2)}|{round(df_agg_views["views_unique"].iloc[x:x+7].mean(), 2)}|
        """
            )
        )
